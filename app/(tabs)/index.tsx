...
export default function HomeScreen() {
  ...
  const [posts, setPosts] = useState<Post[]>([]);  const [newPost, setNewPost] = useState({
    content: '',
    author: '',
  ...
  const handleReaction = (postId: string, emoji: string, isReply: boolean = false, replyId?: string) => {
    setPosts(posts.map(post => {
      if (post.id === postId) {
        if (isReply && replyId) {
          // Reply のリアクション
  ...
  const showReactionPicker = (postId: string, isReply: boolean = false, replyId?: string) => {
    const reactions = ['👍', '❤️', '😊', '😂', '😮', '😢'];
          ...
          <Animated.View style={[
                ...
                <View style={styles.messageItem}>
                  ...
                  <View style={styles.messageIcons}>
                    <TouchableOpacity 
                      ...
                      onPress={() => setReplyMode(replyMode === post.id ? null : post.id)}
                    >
                  ...
                  <View style={styles.reactionsDisplay}>
                    {Object.entries(post.reactionCounts).map(([emoji, count]) => (
                      <TouchableOpacity
                        ...
                        style={[
                          styles.reactionItem,
                          post.reactions?.[currentUserId] === emoji && styles.reactionItemActive
                        ]}
                      ...
                      <View key={reply.id}>
                        <View style={styles.replyItem}>
                          ...
                          <View style={styles.replyContent}>
                            <Text style={styles.replyUserName}>{reply.author}</Text>
                          ...
                          <View style={styles.replyActions}>
                            <TouchableOpacity 
                              ...
                              onPress={() => showReactionPicker(post.id, true, reply.id)}
                            >
                        ...
                        {/* リプライのリアクション表示 */}
                        {reply.reactionCounts && Object.keys(reply.reactionCounts).length > 0 && (
                          <View style={styles.replyReactionsDisplay}>
                            {Object.entries(reply.reactionCounts).map(([emoji, count]) => (
                              <TouchableOpacity
                                key={emoji}
                                style={[
                                  styles.replyReactionItem,
                                  reply.reactions?.[currentUserId] === emoji && styles.reactionItemActive
                                ]}
                                onPress={() => handleReaction(post.id, emoji, true, reply.id)}
                              >
                                <Text style={styles.reactionEmoji}>{emoji}</Text>
                                <Text style={styles.reactionCount}>{count}</Text>
                              </TouchableOpacity>
                ...
                {replyMode === post.id && (
                  <View style={styles.replyForm}>
                    <TextInput
                      ...
                      onChangeText={(text) => setNewReply({ ...newReply, content: text })}
                      multiline={true}
                      maxLength={200}
                    />
                    <TextInput
                      ...
                      onChangeText={(text) => setNewReply({ ...newReply, author: text })}
                      maxLength={20}
                    />
            ...
            {getPostsForLocation().length === 0 && (
              <View style={styles.emptyState}>
                <Text style={styles.emptyStateText}>この場所にはまだコメントがありません</Text>
              </View>
            )}
          </ScrollView>
        </Animated.View>
